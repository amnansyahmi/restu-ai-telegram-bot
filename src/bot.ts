...
export function createBot(token:string,publicUrl:string){
  ...
  bot.command("menu",async ctx=>{if(!ctx.from)return;const profile=await getProfile(ctx.from.id,ctx.from.first_name);if(profile.onboardingStep!=="complete"){await continueOnboarding(ctx);return;}await ctx.reply("What would you like to do?",{reply_markup:mainKeyboard(publicUrl)});});
